...
import io, json, zipfile, re, sys, subprocess, importlib, tempfile, os, hashlib, pickle
...
# V110.3.19.9: base funcional V110.3.19.9; sólo cambia el solver inferior dentro de _fit_skel_sequence().
# La caché queda aislada por registro (fingerprint V104/V107 + sexo) para evitar
# reutilizar accidentalmente una marcha de otro paciente/registro.
_V193_CACHE_DIR = Path("/tmp/physiosentinel_skel_75f_v110_3_19_5")
_V193_CACHE_DIR.mkdir(parents=True, exist_ok=True)

def _v194_motion_fingerprint(motion, gender: str) -> str:
    try:
        frames=list((motion or {}).get("frames") or [])
        sample=[]
        for idx in ([0, len(frames)//2, len(frames)-1] if frames else []):
            fr=frames[max(0,min(len(frames)-1,int(idx)))]
            pts=_frame_points_from_frame(fr)
            row={k:[round(float(x),6) for x in np.asarray(v,float).reshape(-1)[:3]] for k,v in sorted(pts.items())}
            sample.append({"i":int(idx),"pts":row})
        raw=json.dumps({"n":len(frames),"gender":str(gender),"sample":sample},sort_keys=True,separators=(",",":"),ensure_ascii=False).encode("utf-8")
        return hashlib.sha256(raw).hexdigest()[:20]
    except Exception:
        return hashlib.sha256((str(gender)+"|fallback").encode()).hexdigest()[:20]

def _v193_cache_path(kind: str, gender: str) -> Path:
    g = "female" if str(gender).lower().startswith("f") else "male"
    token=str(st.session_state.get("_v194_motion_cache_key") or "no_motion")
    safe=re.sub(r"[^a-zA-Z0-9_-]+","_",token)[:32]
    return _V193_CACHE_DIR / f"{safe}_{kind}_{g}.pkl"

def _v194_manifest_path(gender: str) -> Path:
    return _v193_cache_path("manifest",gender).with_suffix(".json")

def _v194_write_manifest(gender: str, **updates) -> None:
    try:
        pp=_v194_manifest_path(gender)
        data={}
        if pp.exists():
            try: data=json.loads(pp.read_text(encoding="utf-8"))
            except Exception: data={}
        data.update({k:_json_safe_134(v) if '_json_safe_134' in globals() else v for k,v in updates.items()})
        data["version"]="110.3.19.9"
        tmp=pp.with_suffix(pp.suffix+".part")
...
def _v194_read_manifest(gender: str):
    try:
        pp=_v194_manifest_path(gender)
...
def _v193_save(kind: str, gender: str, obj) -> bool:
    """Escritura pickle atómica: .part + fsync + os.replace."""
    pp=_v193_cache_path(kind, gender)
    tmp=pp.with_suffix(pp.suffix+".part")
    try:
...
def _v193_load(kind: str, gender: str):
    try:
        pp=_v193_cache_path(kind, gender)
        if pp.exists() and pp.stat().st_size>32:
...
def _v194_npz_path(gender: str) -> Path:
    return _v193_cache_path("mesh_export",gender).with_suffix(".npz")

def _v194_save_npz_bytes(gender: str, raw: bytes) -> bool:
    pp=_v194_npz_path(gender); tmp=pp.with_suffix(pp.suffix+".part")
    try:
        with open(tmp,"wb") as fh:
            fh.write(raw); fh.flush(); os.fsync(fh.fileno())
        os.replace(tmp,pp); return True
    except Exception:
        try: tmp.unlink(missing_ok=True)
        except Exception: pass
        return False
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    # V110.3.18: perfil temporal real de miembros inferiores. Se calcula una sola vez
    # sobre toda la secuencia y conserva qué ejes contienen el movimiento observado.
    ...
    for fi in range(int(start_index),len(frames)):
        ...
        lower_temporal={'accepted':False,'reason':'frame semilla'}
        if fi>int(start_index):
            ...
            # V110.3.18: rescate explícito de miembros inferiores. Sólo commit si
            # mejora la fidelidad de cadera/rodilla/tobillo en el plano observado.
            if prev_tdf is not None:
                pose,lower_temporal=_lower_limb_delta_tracking_199_fast(torch,model,pose,rotvec,trans,scale,tdf,prev_tdf,betas,zero_trans,prev_pose,axis_map,lower_profile,acquisition_mode)
            else:
                pose,lower_temporal=_lower_limb_vector_dls_196(torch,model,pose,rotvec,trans,scale,tdf,rows,betas,zero_trans,prev_pose,axis_map,lower_profile,acquisition_mode)
        with torch.no_grad():
            world,_,_=_direct_world_130(torch,model,pose,betas,zero_trans,rotvec,scale,trans,axis_map=axis_map)
            pred=world.index_select(0,idx); errors=torch.linalg.norm(pred-tgt,dim=1); rmse=float(torch.sqrt(torch.mean(errors*errors)).cpu())
            _d=(pred-tgt).cpu().numpy()
            rmse_xy=float(np.sqrt(np.mean(np.sum(_d[:,:2]**2,axis=1))))
            rmse_z=float(np.sqrt(np.mean(_d[:,2]**2)))
            audit=_direct_anatomical_audit_130(model,world.cpu().numpy(),pose.cpu().numpy()[0],tdf)
            _dw=float(tdf.attrs.get('depth_weight',1.0)) if hasattr(tdf,'attrs') else 1.0
            depth_gate=_acquisition_mode_gate_142(rmse_xy,rmse_z,audit,acquisition_mode,xy_limit=0.38)
            sequence.append({'frame':fi+1,'status':'ok','n_correspondences':len(rows),'rmse':rmse,'rmse_xy':rmse_xy,'rmse_z':rmse_z,'anatomical_audit':audit,'depth_safe_validation':depth_gate,
                'retargeting_mode':'official_skel24_peripheral_refinement','active_q_names':[_SKEL_Q_NAMES_130[i] for i in _active_for_target_133(tdf)],
                ...
                'trans':trans.cpu().numpy().astype(float).tolist(),'joints':world.cpu().numpy().astype(float).tolist(),'lower_limb_temporal':lower_temporal,
                'lower_limb_q':{_SKEL_Q_NAMES_130[i]:float(pose[0,i].cpu()) for i in [3,4,5,6,7,10,11,12,13,14]}})
            prev_pose=pose.clone(); prev_rot=rotvec.clone(); prev_trans=trans.clone(); prev_tdf=tdf.copy()
        if progress_cb: progress_cb(fi+1,len(frames),rmse)
    result={'version':'110.3.19.9','retargeting_mode':'official_skel24_lower_limb_fast_frame_delta_tracking','scale':float(scale.cpu()),
    ...
    # V110.3.19.9: las auditorías posteriores a 75/75 son informativas y NUNCA
    # pueden impedir el handoff 75F -> skin_verts. Si una auditoría falla, se
    # conserva la secuencia y se registra el error para inspección.
    try:
        result['pose_sequence_integrity']=_pose_sequence_integrity_audit_147(result,lower_profile)
    except Exception as _audit_exc:
        result['pose_sequence_integrity']={'ok':False,'error':f'{type(_audit_exc).__name__}: {_audit_exc}','non_blocking':True}
    try:
        result['lower_limb_motion_gate']=_lower_limb_motion_gate_147(result,lower_profile)
    except Exception as _gate_exc:
        result['lower_limb_motion_gate']={'ok':False,'error':f'{type(_gate_exc).__name__}: {_gate_exc}','non_blocking':True}
...
def _build_skel_skin_sequence(torch, model, seq, progress_cb=None):
    ...
    return {
        ...
        "migration_source":dict(st.session_state.get("v110_3_18_npz_migration_source") or {}),
    }
...
def _mesh_sequence_npz_bytes(mesh_seq):
    ...
    payload={
        ...
        "migration_source":np.asarray([json.dumps(_json_safe_134(mesh_seq.get("migration_source",{})),ensure_ascii=False)]),
    }
...
def _plot_skel_mesh_walker(mesh_seq):
    ...
    try:
        ...
        const layout={{title:'V110.3.19.9 · SKEL ANIMATED ANATOMICAL MESH · pelvis-centered playback',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}},legend:{{orientation:'h'}}}};
        ...
        st.caption("V110.3.19.9 · visor centrado en pelvis sólo para presentación. El NPZ científico conserva las coordenadas originales. Usa ‘⬇ Vídeo SKEL’ para grabar y descargar la marcha anatómica desde el visor.")
    except Exception as exc:
...
def _mesh_sequence_from_npz_bytes_145(raw: bytes):
    """Carga una marcha SKEL ya calculada sin reejecutar fitting/75 frames."""
    with np.load(io.BytesIO(raw),allow_pickle=False) as z:
        required={"vertices","faces","frame_ids"}
        miss=required-set(z.files)
        if miss: raise RuntimeError("NPZ SKEL incompleto: falta "+", ".join(sorted(miss)))
        V=np.asarray(z["vertices"],dtype=np.float32); F=np.asarray(z["faces"],dtype=np.int32)
        if V.ndim!=3 or V.shape[-1]!=3 or V.shape[0]<2 or V.shape[1]<1000:
            raise RuntimeError(f"vertices inválidos: {V.shape}")
        if F.ndim!=2 or F.shape[1]<3 or F.min()<0 or F.max()>=V.shape[1]:
            raise RuntimeError("faces inválidas")
        def scalar(name,default=""):
            if name not in z.files: return default
            a=np.asarray(z[name]).reshape(-1)
            return a[0].item() if a.size else default
        J=np.asarray(z["joints"],dtype=np.float32) if "joints" in z.files else None
        if J is not None and (J.ndim!=3 or J.shape[0]!=V.shape[0]): J=None
        Poses=np.asarray(z["poses"],dtype=np.float32) if "poses" in z.files else None
        if Poses is not None and (Poses.ndim!=2 or Poses.shape[0]!=V.shape[0] or Poses.shape[1]<46): Poses=None
        gender=str(scalar("skel_gender",scalar("gender","unknown")))
        out={
            "version":str(scalar("version","110.3.19.3-import")),
            "frame_ids":np.asarray(z["frame_ids"],dtype=np.int16),
            "vertices":V,"faces":F[:,:3],"joints":J,"poses":Poses,
            "joint_names":[str(x) for x in np.asarray(z["joint_names"]).tolist()] if "joint_names" in z.files else [],
            "scale":float(scalar("scale",1.0)),
            "betas":np.asarray(z["betas"],dtype=np.float32) if "betas" in z.files else np.zeros((10,),np.float32),
            "face_source":"NPZ importado",
            "source_sequence_version":str(scalar("source_sequence_version","")),
            "source_sequence_scale":float(scalar("source_sequence_scale",scalar("scale",1.0))),
            "axis_map":np.asarray(z["axis_map"],dtype=np.float32) if "axis_map" in z.files else np.eye(3,dtype=np.float32),
            "skel_gender":gender,
            "migration_source":str(scalar("migration_source","")),
            "imported_npz":True,
        }
        return out
...
def _lower_limb_vector_dls_196(torch,model,pose,rotvec,trans,scale,target_df,rows,betas,zero_trans,prev_pose,axis_map,profile,acquisition_mode):
    """V110.3.19.9 · propagación q(t) de miembros inferiores basada en VECTORES DE SEGMENTO.
    ...
    audit={'accepted':False,'strategy':'V110.3.19.9 relative-segment weighted-XYZ DLS','axis_weights':w.astype(float).tolist(),'chains':{}}
    any_commit=False
...
def _lower_limb_delta_tracking_199_fast(torch,model,pose,rotvec,trans,scale,target_df,prev_target_df,betas,zero_trans,prev_pose,axis_map,profile,acquisition_mode):
    """V110.3.19.9 · solver inferior quirúrgico y rápido.

    Base V110.3.19.9 intacta fuera de esta función. Sigue el delta frame-a-frame de
    ...
    try:
        ...
        chains=[('R',[3,4,5,6],('RHip','RKnee','RAnkle'),('femur_r','tibia_r','talus_r')),
                ('L',[10,11,12,13],('LHip','LKnee','LAnkle'),('femur_l','tibia_l','talus_l'))]
        audit={'accepted':False,'strategy':'V110.3.19.9 fast frame-delta DLS','axis_weights':w.tolist(),'chains':{}}
        with torch.no_grad():
        ...
        return pose,{'accepted':False,'strategy':'V110.3.19.9 fast frame-delta DLS','error':f'{type(exc).__name__}: {exc}','non_blocking':True}
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    frames=list((motion or {}).get("frames") or [])
    # V110.3.19.9: namespace de persistencia ligado a ESTE registro. Se recalcula
    # tras resolver sexo, pero ya dejamos un fingerprint provisional por movimiento.
    st.session_state["_v194_motion_cache_key"]=_v194_motion_fingerprint(motion, st.session_state.get("patient_sex","unknown"))
    if not frames:
    ...
    st.markdown("### ⚡ Abrir o migrar una marcha SKEL ya calculada")
    st.caption("V110.3.19.9 separa reproducción y recálculo y recupera automáticamente resultados 75F/malla ya finalizados. Un NPZ antiguo puede reproducirse tal cual o usarse sólo como referencia mientras el motor actual recalcula la marcha desde los 75 frames V104/V107 cargados en esta sesión.")
    _npz=st.file_uploader("Marcha SKEL existente (.NPZ)",type=["npz"],key="v110_3_18_direct_mesh_npz")
    if _npz is not None:
        try:
            _npz_raw=_npz.getvalue()
            _digest=hashlib.sha256(_npz_raw).hexdigest()[:16]
            _digest_key="v110_3_18_direct_npz_digest"
            _mode_key="v110_3_18_direct_npz_mode"
            if st.session_state.get(_digest_key)!=_digest:
                st.session_state[_digest_key]=_digest
                st.session_state[_mode_key]=None
            _loaded=_mesh_sequence_from_npz_bytes_145(_npz_raw)
            _pg,_ps=_patient_gender_145(); _fg=str(_loaded.get('skel_gender','unknown')).lower()
            _srcv=str(_loaded.get('source_sequence_version') or _loaded.get('version') or 'desconocida')
            _has_poses=isinstance(_loaded.get('poses'),np.ndarray) and _loaded['poses'].ndim==2 and _loaded['poses'].shape[1]>=46
            _legacy=(_fg not in {'male','female'}) or (not _has_poses) or (_srcv not in {'110.3.17','110.3.18'})
            _la=_audit_skel_animated_mesh(_loaded)
            st.success(f"NPZ detectado · {_la.get('frames',0)} frames · {_la.get('vertices_per_frame',0)} vértices/frame · versión fuente {_srcv} · sexo malla: {_fg if _fg!='unknown' else 'no registrado'}")
            if _legacy:
                st.warning("⚠️ NPZ legado detectado. Puede reproducirse, pero no contiene toda la trazabilidad de pose/sexo del motor actual. Para corregir movimiento de piernas hay que RECALCULAR desde la secuencia V104/V107 de esta sesión; el NPZ por sí solo no contiene los landmarks objetivo necesarios.")
            b1,b2=st.columns(2)
            if b1.button("▶️ Reproducir NPZ existente",use_container_width=True,key="v110_3_18_play_existing_npz"):
                st.session_state[_mode_key]='play'
            if b2.button("🔄 Recalcular marcha con motor V110.3.18",type="primary",use_container_width=True,key="v110_3_18_recalculate_from_motion"):
                st.session_state[_mode_key]='recalc'
                st.session_state['v110_3_18_npz_migration_source']={
                    'digest':_digest,'source_version':_srcv,'source_gender':_fg,'legacy':bool(_legacy)
                }
                # El NPZ NO se usa como fuente cinemática. Se reutilizan los 75 frames V104/V107
                # que ya están activos en esta sesión y se ejecuta el motor temporal actual.
                for _k in ['v110_3_18_sequence','v110_3_18_mesh_sequence','v110_3_18_mesh_audit']:
                    st.session_state.pop(_k,None)
            _mode=st.session_state.get(_mode_key)
            if _mode=='play':
                if _fg in {'male','female'} and _pg in {'male','female'} and _fg!=_pg:
                    st.error(f"El NPZ fue generado con SKEL {_fg}, pero la ficha actual indica {_ps}. No se mezclan identidades sexuales silenciosamente.")
                    return
                st.success("▶️ Modo reproducción directa: no se recalcula Frame 1, IK, poses ni skin_verts.")
                _plot_skel_mesh_walker(_loaded)
                st.download_button("⬇️ Reexportar marcha SKEL cargada (NPZ)",_mesh_sequence_npz_bytes(_loaded),"V110_3_19_5_SKEL_mesh_sequence.npz","application/octet-stream",use_container_width=True)
                return
            elif _mode=='recalc':
                st.success(f"🔄 Modo RECÁLCULO activo · se ignorará la pose/malla del NPZ {_srcv} y se reconstruirá desde los {len(frames)} frames V104/V107 actuales con V110.3.18. Sexo aplicado desde la ficha: {_ps}.")
                st.caption("No necesitas retirar el NPZ. Al finalizar podrás descargar un NPZ nuevo con `poses[frame,46]`, sexo SKEL y auditoría temporal actualizados.")
            else:
                st.info("Elige una opción: reproducir el NPZ tal como está o recalcular la marcha con el motor actual. V110.3.18 ya no entra automáticamente en reproducción al cargar un NPZ.")
                return
        except Exception as _npz_exc:
            st.error(f"NPZ SKEL no válido: {type(_npz_exc).__name__}: {_npz_exc}")
    ...
    st.markdown("**Modelo SKEL privado · V110.3.19.9 · Lower-Limb q(t) Propagation Fix**")
    ...
    if not runtime:
        ...
        st.session_state["_v194_motion_cache_key"]=_v194_motion_fingerprint(motion, gender or patient_sex_label)
        if gender is None:
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                try:
                    ...
                    export={
                        "version":"110.3.19.9","frame":int(best_i)+1,"gender":gender,"rmse_before":fit["rmse_before"],"rmse_after":fit["rmse_after"],
                    ...
                    st.markdown("### V110.3.19.9 · Propagación temporal 1→75")
                    ...
                    _already_75f = isinstance(st.session_state.get("v110_3_18_sequence"),dict) or (_v193_load("sequence", gender) is not None)
                    if _already_75f:
                        st.success("✅ Los 75 frames ya están calculados. No se repetirán. La marcha SKEL se abre debajo automáticamente.")
                        ...
                        try:
                            ...
                            if len(oks_auto)>=max(2,int(0.9*expected)):
                                ...
                                try:
                                    ...
                                    # Persistencia POSTERIOR y best-effort. Ningún fallo aquí puede
                                    # borrar la secuencia o la malla ya generadas.
                                    _persist_errors=[]
                                    try:
                                        _seq_saved=_v193_save("sequence", gender, seq)
                                    except Exception as _e:
                                        _seq_saved=False; _persist_errors.append(f"sequence: {type(_e).__name__}: {_e}")
                                    try:
                                        _mesh_saved=_v193_save("mesh", gender, mesh_seq)
                                    except Exception as _e:
                                        _mesh_saved=False; _persist_errors.append(f"mesh: {type(_e).__name__}: {_e}")
                                    try:
                                        _audit_saved=_v193_save("mesh_audit", gender, mesh_audit)
                                    except Exception as _e:
                                        _audit_saved=False; _persist_errors.append(f"audit: {type(_e).__name__}: {_e}")
                                    try:
                                        _npz_raw=_mesh_sequence_npz_bytes(mesh_seq)
                                        _npz_saved=_v194_save_npz_bytes(gender,_npz_raw)
                                    except Exception as _e:
                                        _npz_saved=False; _persist_errors.append(f"npz: {type(_e).__name__}: {_e}")
                                    try:
                                        _v194_write_manifest(gender,status="mesh_ready",sequence_saved=bool(_seq_saved),mesh_saved=bool(_mesh_saved),mesh_audit_saved=bool(_audit_saved),npz_saved=bool(_npz_saved),frames=int(np.asarray(mesh_seq.get("vertices")).shape[0]))
                                    except Exception as _e:
                                        _persist_errors.append(f"manifest: {type(_e).__name__}: {_e}")
                                    if _persist_errors:
                                        st.warning("La malla está generada y disponible, pero alguna persistencia secundaria falló: " + " | ".join(_persist_errors))
                                    ...
                                    try:
                                        _v193_save("sequence", gender, seq)
                                    except Exception:
                                        pass
                            else:
                    ...
                    if not (isinstance(seq,dict) and seq.get("frames")):
                        seq=_v193_load("sequence", gender)
                        if isinstance(seq,dict) and seq.get("frames"):
                            st.session_state["v110_3_18_sequence"]=seq
                            st.success("✅ Resultado 75F recuperado automáticamente de persistencia atómica. No es necesario volver a procesar los 75 frames.")
                    if isinstance(seq,dict) and seq.get("frames"):
                        # V110.3.19.9 · RESTAURACIÓN DEL FLUJO AUTOMÁTICO:
                        # una secuencia 75F válida nunca debe quedarse esperando un segundo botón.
                        # Si falta la malla tras un rerun/reconexión, se reconstruye automáticamente
                        # desde q(t) persistido, sin repetir el fitting 1→75.
                        ...
                        if not (isinstance(_mesh_now,dict) and isinstance(_mesh_now.get("vertices"),np.ndarray)):
                            _mesh_now=_v193_load("mesh", gender)
                            if isinstance(_mesh_now,dict) and isinstance(_mesh_now.get("vertices"),np.ndarray):
                                st.session_state["v110_3_18_mesh_sequence"]=_mesh_now
                                _cached_audit=_v193_load("mesh_audit", gender)
                                if isinstance(_cached_audit,dict): st.session_state["v110_3_18_mesh_audit"]=_cached_audit
                                st.success("✅ Marcha SKEL 75F recuperada automáticamente. Reproductor y descargas disponibles debajo.")
                        if not (isinstance(_mesh_now,dict) and isinstance(_mesh_now.get("vertices"),np.ndarray)):
                            ...
                            if len(_ok_auto)>=max(2,int(0.9*max(1,len(frames)-int(best_i)))):
                                if _ll_auto and not bool(_ll_auto.get('ok',True)):
                                    st.warning("⚠️ Auto-recovery: la auditoría de miembros inferiores pide revisión, pero NO bloquea la reconstrucción de malla en V110.3.19.9.")
                                st.info("75F ya calculado. Generando automáticamente skin_verts y malla SKEL; NO se repite el retargeting temporal.")
                                _auto_bar=st.progress(0,text="Auto-recovery · generando malla desde poses 75F persistidas…")
                                def _recovery_mesh_cb(done,total,nverts):
                                    _auto_bar.progress(min(1.0,float(done)/max(1,total)),text=f"Auto-recovery malla {done}/{total} · {nverts} vértices")
                                try:
                                    ...
                                    _mesh_saved=_v193_save("mesh",gender,_mesh_now)
                                    _audit_saved=_v193_save("mesh_audit",gender,_mesh_audit_now)
                                    _npz_raw=_mesh_sequence_npz_bytes(_mesh_now)
                                    _npz_saved=_v194_save_npz_bytes(gender,_npz_raw)
                                    _v194_write_manifest(gender,status="mesh_ready",mesh_saved=bool(_mesh_saved),mesh_audit_saved=bool(_audit_saved),npz_saved=bool(_npz_saved),frames=int(np.asarray(_mesh_now.get("vertices")).shape[0]),recovered_from_sequence=True)
                                    _auto_bar.progress(1.0,text="Auto-recovery completado · malla SKEL lista")
                                    st.success("✅ Flujo automático restaurado: 75F → skin_verts → malla → reproductor → NPZ.")
                                except Exception as _rec_exc:
                                    _v194_write_manifest(gender,status="sequence_ready_mesh_failed",mesh_error=f"{type(_rec_exc).__name__}: {_rec_exc}")
                                    st.error("La secuencia 75F está recuperada, pero la malla automática falló. No se repetirán los 75 frames.")
                                    st.code(f"{type(_rec_exc).__name__}: {_rec_exc}")
                            elif not bool(_ll_auto.get('ok',False)):
                                st.error("⛔ Secuencia 75F recuperada, pero Temporal Motion Gate no permite construir una falsa marcha. No se repetirán automáticamente los 75 frames.")
                        ...
                        st.download_button("⬇️ Descargar secuencia SKEL V110.3.13 (JSON)",json.dumps(_json_safe_134(export_seq),ensure_ascii=False).encode("utf-8"),"V110_3_19_5_SKEL_sequence.json","application/json",use_container_width=True)
                        ...
                        st.markdown("### V110.3.19.9 · SKEL ANIMATED ANATOMICAL MESH")
                        ...
                        if st.button("🔁 Regenerar malla anatómica SKEL",use_container_width=True,key="v110_3_13_build_skin_mesh"):
                            ...
                            try:
                                ...
                                _v193_save("mesh", gender, mesh_seq)
                                _v193_save("mesh_audit", gender, st.session_state["v110_3_18_mesh_audit"])
                                _npz_raw=_mesh_sequence_npz_bytes(mesh_seq)
                                _v194_save_npz_bytes(gender,_npz_raw)
                                _v194_write_manifest(gender,status="mesh_ready",frames=int(np.asarray(mesh_seq.get("vertices")).shape[0]),manual_regeneration=True)
                        ...
                        if not (isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray)):
                            mesh_seq=_v193_load("mesh", gender)
                            if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                                st.session_state["v110_3_18_mesh_sequence"]=mesh_seq
                                _cached_audit=_v193_load("mesh_audit", gender)
                                if isinstance(_cached_audit,dict): st.session_state["v110_3_18_mesh_audit"]=_cached_audit
                                st.success("✅ Malla SKEL 75F recuperada de caché. Abriendo directamente la marcha calculada.")
                        if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                            ...
                            _v194_save_npz_bytes(gender,_npz_bytes)
                            st.download_button("⬇️ DESCARGAR MARCHA SKEL 75F (.NPZ)",_npz_bytes,"V110_3_19_5_SKEL_mesh_sequence.npz","application/octet-stream",use_container_width=True,type="primary")
                            st.caption("NPZ científico persistente: vertices[frame,6890,3], faces, joints, poses[frame,46] cuando están disponibles, frame_ids, escala, betas y sexo SKEL. Guárdalo para abrir la marcha directamente sin recalcular 75F.")
                            _man=_v194_read_manifest(gender)
                            if _man:
                                st.caption(f"Persistencia V110.3.19.9: {_man.get('status','—')} · frames={_man.get('frames','—')} · NPZ={'OK' if _man.get('npz_saved',True) else 'pendiente'}")
                            st.markdown('### V110.3.19.9 · Validación sincronizada Side-by-Side')
                        ...
                        if frame_gate_ok:
                            st.info("✅ El frame semilla está validado. Pulsa el botón anterior para resolver frames 2→75; después V110.3.19.9 generará automáticamente skin_verts(t) y la malla corporal animada en la misma ejecución.")
                        else:
